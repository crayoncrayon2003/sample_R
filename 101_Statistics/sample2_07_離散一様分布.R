main <- function() {
  ...
  # 例１：サイコロを1回振ったときに出る目（1〜6）は、どの値も同じ確率 1/6 で出現するため、離散一様分布に従う
  # 例２：番号札 1〜6 を箱に入れて1枚引くとき、どの番号も同じ確率で選ばれるため、離散一様分布に従う
  #
  # コイン投げや不良品の発生は「成功・失敗」の確率が決まっているため、一様分布ではない。
  # 一方、出る値があらかじめ決まった整数範囲で、すべて同じ確率で起こる状況では離散一様分布である。
