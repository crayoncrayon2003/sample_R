main <- function() {
  ...
  # まとめ
  # =====================================================
  # 因果推論の観測と介入の流れは、次のようになる。
  #    Step1 : 観測データとドメイン知識を用いて、因果ダイアグラム・SCMを仮設する。
  #    Step2 : 観測データをSCMに当てはめた時、処置変数に対する平均因果効果が否定されないことを確認する。
  # 　   　　　 これでSCMの正しさは確認できないが、最低限SCMを制約とみなした時に観測データが矛盾していないことを確認する。
  #    Step3 : SCMが矛盾している場合、Step1 に戻ってやり直す。
  # 　   　　　 SCMが矛盾していない場合、Step4 に進む
  #    Step4 : 仮説した因果ダイアグラム・SCMを用いて、どの変数・どの範囲・どう操作すると望ましい結果になりそうか評価する
  #    Step5 : 介入による平均因果効果を確認する。

  # 疑問１
  # Step1で、因果ダイアグラム・SCMを仮設している。
  # バックドア基準や、共変量、未知の共変量が、存在している。この中で、より確からしい因果ダイアグラム・SCMの仮設は、出来ない気がする。
  # 　（仮説だと言っている因果ダイアグラム・SCMに対して、その仮説がおかしいという主張自体が変な気もするけど）
  #
  # ⇒　バックドア基準は、因果ダイアグラム を作るための道具である。「評価基準」であって「発見方法」ではない。
  # 　　この 因果ダイアグラム が正しいと仮定したときに、どの変数を調整すれば因果効果が識別できるか。
  # 　　未知の共変量の存在は常に否定できない。しかし、「隠れ交絡が存在しない」という仮定は検証できない。
  # 　　この 因果ダイアグラム には未観測交絡がないという前提の基でしか成り立たない。
  #
  # 　　このため、因果推論は、「この仮定を置くなら、この結論が導かれる」という条件付き主張 を厳密に実施している。
  # 　　実務は、複数の因果ダイアグラム・SCMを仮設して、それらを比較しながら同時に評価することになる。

  # 疑問２
  # Step1は、観測データとドメイン知識を用いて、因果ダイアグラム・SCMを仮設している。
  # Step4は、仮設した因果ダイアグラム・SCMを用いて、介入した時に、どの変数・どの範囲・どう操作すると望ましい結果になりそうか評価している。
  #
  # Step1の仮説に対して、Step4で更に仮説を重ねている気がする。
  # 〇ケース１
  #     DAG・SCMパターン1を仮設 ⇒ 変数Aに対して介入して評価結果1（これも仮説）
  #     DAG・SCMパターン2を仮設 ⇒ 変数Aに対して介入して評価結果2（これも仮説）
  #     ...
  #     DAG・SCMパターンnを仮設 ⇒ 変数Aに対して介入して評価結果n（これも仮説）
  #
  # このようになる気がする。評価結果1・評価結果2・評価結果nは、ぞれぞれ前提条件が異なるため、そもそも比較できないのではないか。
  # 一方、以下は、比較できると思う。なぜなら、介入の前提条件が同じだから。
  # 〇ケース２
  #     DAG・SCMパターン1を仮設 ⇒ 変数Aに対して介入して評価結果1（これも仮説）
  #     DAG・SCMパターン1を仮設 ⇒ 変数Bに対して介入して評価結果2（これも仮説）
  #     ...
  #     DAG・SCMパターン1を仮設 ⇒ 変数nに対して介入して評価結果n（これも仮説）
  #
  # ⇒　基本的に正しい。
  # 　　ケース2は、厳密に比較できる
  # 　　ケース1は、厳密に比較できない。ただし、因果推論でやりたいことは、どの介入が“頑健そうか”を探すこと。
  # 　　例えば、次のような結果になった場合、変数Bよりも変数Aに介入した方が、良さそうであることは分かる。
  # 　　DAG・SCMパターン１を仮設 ⇒ 変数Aに対して介入して結果に影響しそうだ
  # 　　DAG・SCMパターン２を仮設 ⇒ 変数Aに対して介入して結果に影響しそうだ
  # 　　DAG・SCMパターンｎを仮設 ⇒ 変数Aに対して介入して結果に影響しそうだ

  # 　　DAG・SCMパターン１を仮設 ⇒ 変数Bに対して介入して結果に影響しなさそうだ
  # 　　DAG・SCMパターン２を仮設 ⇒ 変数Bに対して介入して結果に影響しなさそうだ
  # 　　DAG・SCMパターンｎを仮設 ⇒ 変数Bに対して介入して結果に影響しなさそうだ
